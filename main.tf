...
resource "aws_subnet" "private_subnets" {
  ...
  availability_zone = "${data.aws_availability_zones.available.names[count.index]}"
  tags              = "${merge(var.tags, map("Name", format("%s-private-subnet-%s", var.name, element(data.aws_availability_zones.available.names, count.index))), map("immutable_metadata", format("{\"purpose\":","demo-public-subnet",count.index,"}")))}"
}
